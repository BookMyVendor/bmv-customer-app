...
export default function BulkQuoteModal({
  ...
  const handleSubmit = async () => {
    ...
    if (vendors.length === 0) {
      Alert.alert('No Vendors', 'Please select at least one vendor.');
      return;
    }
    ...
    try {
      ...
      const results = await Promise.all(
        vendors.map(vendor => 
          ...
          .catch(err => ({ success: false, error: err.message, businessName: vendor.business_name }))
        )
      );
      ...
      if (failures.length === 0) {
        ...
        const failedNames = failures.map(f => f.businessName || 'Unknown').join(', ');
        Alert.alert('Partial Success', `Sent quotes to most vendors, but failed for: ${failedNames}. Please try again later for these.`);
        onClose();
      }
    } catch (error: any) {
      Alert.alert('Error', error.message || 'Failed to submit quote requests');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          <View style={styles.modalContent}>
            <View style={styles.header}>
              <Text style={styles.headerTitle}>Get Bulk Quotes</Text>
            ...
            <View style={styles.footer}>
              ...
              >
                {loading ? (
                  ...
                  <>
                    <Text style={styles.submitButtonText}>Send Request to {vendors.length} Vendors</Text>
